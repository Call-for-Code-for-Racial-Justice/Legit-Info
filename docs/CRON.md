...
Since Legiscan.com API only updates their bulk data downloads on Sunday,
you can run this once per week with cron.  In this example, the weekly.sh
script is started at 12:30pm every Sunday.

```console
30 12 * * 0  /home/yourname/Dev/Legit-Info/weekly.sh
```

The weekly.sh script has three phases:
...
You can invoke this in either "on demand" or as part of the "weekly.sh"
cron job.
...
to run the job natively.
```console
[legit-info]$ ./cron1 extract_files --api --state AZ
```

If you are in the Pipenv shell, you can use stage1 or stage2, as well.

```console
(cfc) [legit-info]$ ./stage1 extract_files --api --state AZ
```
...
terms.  A relevant term can be a single word or a phrase.  Some of the phrases
might have specific meanings in judicial, legislative, medical, or other 
industry-specific terminology.

### 3.1 Processing Logic

Each text file is read.  The lines are merged into a single line, and 
all quotation marks are converted to pipe character to avoid confusing
IBM Watson NLU algorithm.  The response is parsed for the top 10 concepts,
matching against the words and phrases in the /sources/wordmap.csv file.

If --api is not specified, the alternative method is to use the words and
phrases in the /sources/wordmap.csv to chose an appropriate impact.

The cfc_app_law database table is updated with the impact chosen, as well
as the relevant words and phrases used in that AI decision.
...
If you leave out the --api, the IBM Watson NLU will not be invoked.
Instead, the /sources/wordmap.csv file will be used to select the
impact using pattern matching.

You can use cron1 or cron2 scripts to set up the Pipenv environment
to run the job natively.

```console
[legit-info]$ ./cron1 analyze_text --api --skip
```

If you are in the Pipenv shell, you can use stage1 or stage2, as well.

```console
(cfc) [legit-info]$ ./stage1 analyze_text --api --compare
```
...
If you do not specify --api, then legislation will be processed with
pattern matching using /sources/wordmap.csv instead.  The --skip 
parameter will skip any existing legislation that has already an
entry in the cfc_app_law database table with impact/relevance already
from a previous run.  If --compare is chosen, then both IBM Watson NLU
and pattern matching methods are used, and will be reflected in the 
relevance field of the cfc_app_law database table.


```
[legit-info]$ ./cron1 analyze_text --api --skip --compare
**Using SQLite3**
Starting analyze_text at Nov-10 05:09PM MST
Processing: United States (US)
...............................
...
Processing: Ohio (OH)
...............................
Ending analyze_text at Nov-10 05:10PM MST
```
